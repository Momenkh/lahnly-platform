...
SEPARATION_MODEL_BASE_CONF = {
    # htdemucs_6s dedicated stems (6-stem model — one pass yields all six)
    ("htdemucs_6s", "guitar"): 0.85,
    ("htdemucs_6s", "bass"):   0.82,
    ("htdemucs_6s", "drums"):  0.80,
    ("htdemucs_6s", "vocals"): 0.78,
    ("htdemucs_6s", "piano"):  0.72,   # lower prior — piano stem noisier on non-piano songs
    ("htdemucs_6s", "other"):  0.55,
    # htdemucs 4-stem fallbacks
    ("htdemucs_ft_other", "other"): 0.60,
    ("htdemucs",          "bass"):  0.55,
    ("htdemucs",          "drums"): 0.55,
    ("htdemucs",          "vocals"):0.55,
    ("htdemucs",          "other"): 0.45,
    # Raw mix fallback: no separation at all
    ("raw mix", "mix"): 0.20,
}
...
# ── stem_confidence formula weights ──────────────────────────────────────────
# confidence = base_conf × CONF_BASE_WEIGHT + energy_ratio × CONF_ENERGY_WEIGHT
# base_conf comes from SEPARATION_MODEL_BASE_CONF (model-level prior).
# energy_ratio = stem_rms / mix_rms (how much energy survived separation).
# The 0.7/0.3 split emphasises the model-level prior over the raw energy ratio
# because energy can be deceiving (a very quiet but clean stem vs a loud bleed).
SEPARATION_CONF_BASE_WEIGHT   = 0.7
SEPARATION_CONF_ENERGY_WEIGHT = 0.3

# Peak normalisation target after separation/blending.
# 0.9 leaves 10% headroom to avoid clipping when effects are added downstream.
SEPARATION_PEAK_NORM_TARGET   = 0.9
