"""
Stage 2 (Bass): Pitch Extraction
Input:  bass stem WAV
Output: raw notes [{pitch, start, duration, confidence}]
        saved to outputs/02_bass_raw_notes.json

Backend: basic-pitch (Spotify) — same model as guitar; bass fundamentals are
very clear for the model at lower frequencies.
CREPE overlay is applied for all bass styles (bass is monophonic/duo-phonic,
making it ideal for the CNN pitch tracker).
"""

import json
import os
import numpy as np

from pipeline.config import get_instrument_dir
from pipeline.settings import (
    BASS_MIDI_MIN,
    BASS_MIDI_MAX,
    BASS_HZ_MIN,
    BASS_HZ_MAX,
    BP_SAMPLE_RATE,
    BP_HOP_LENGTH,
    BP_MIDI_OFFSET,
    BASS_MULTI_PASS_CONFIGS,
    BASS_PITCH_THRESHOLDS,
    BASS_PITCH_ONSET_BASE,
    BASS_PITCH_ONSET_SCALE,
    BASS_PITCH_FRAME_BASE,
    BASS_PITCH_FRAME_SCALE,
    BASS_PITCH_MERGE_PROXIMITY_S,
    BASS_CONF_WEIGHT_BASE,
    BASS_CONF_WEIGHT_CONFIRMED,
    BASS_CONF_WEIGHT_STRONG,
    BASS_PYIN_MIN_NOTE_DURATION_S,
    BASS_CREPE_ENABLED,
    BASS_CREPE_MODEL,
    BASS_CREPE_FMAX,
    BASS_CREPE_CONF_THRESHOLDS,
    BASS_CREPE_MIN_NOTE_S,
    BASS_CREPE_MAX_GAP_S,
    BASS_CREPE_PITCH_TOLERANCE,
    BASS_CREPE_REPET_ENABLED,
    BASS_CREPE_REPET_AUTO_THRESHOLD,
    BASS_SUSTAINED_BOOST_MIN_S,
    BASS_SUSTAINED_BOOST_AMOUNT,
    BASS_SUSTAINED_BOOST_CAP,
)

BP_FRAMES_PER_SEC = BP_SAMPLE_RATE / BP_HOP_LENGTH   # ~86.1 fps


def extract_pitches_bass(
    audio_path: str,
    bass_style: str = "bass_fingered",
    save: bool = True,
) -> list[dict]:
    """
    Extract notes from a bass stem.
    Loads stem_confidence from Stage 1 metadata to tune thresholds.
    bass_style: "bass_fingered" | "bass_picked" | "bass_slap"
    Returns list of {pitch, start, duration, confidence}.
    """
    from pipeline.shared.presence import check_and_update_stem_presence
    if not check_and_update_stem_presence("bass"):
        if save:
            _save_bass([])
        return []

    try:
        return _extract_basic_pitch_bass(audio_path, bass_style=bass_style, save=save)
    except ImportError as e:
        print(f"[Stage 2B] basic-pitch not available ({e}), falling back to pyin")
        return _extract_pyin_bass(audio_path, save=save)


# ── basic-pitch backend ───────────────────────────────────────────────────────

def _extract_basic_pitch_bass(
    audio_path: str,
    bass_style: str = "bass_fingered",
    save: bool = True,
) -> list[dict]:
    from basic_pitch.inference import run_inference, AUDIO_SAMPLE_RATE, FFT_HOP
    from basic_pitch.inference import infer as bp_infer
    from basic_pitch import ICASSP_2022_MODEL_PATH
    from pipeline.shared.separation import load_stem_meta_for, gate_notes_by_stem_energy_for

    stem_conf = load_stem_meta_for("bass").get("stem_confidence", 0.5)
    print(f"[Stage 2B] Loading audio: {audio_path}")
    print(f"[Stage 2B] stem_confidence={stem_conf:.2f}  style={bass_style}")

    # Determine pass configurations
    pass_configs = BASS_MULTI_PASS_CONFIGS.get(bass_style)
    if pass_configs is None:
        onset, frame, min_ms = _resolve_thresholds_bass(bass_style, stem_conf)
        pass_configs = [(onset, frame, min_ms)]

    n_passes = len(pass_configs)

    # Run model inference ONCE — all passes share the same neural network output.
    print(f"[Stage 2B] Running model inference...")
    model_output = run_inference(audio_path, ICASSP_2022_MODEL_PATH)
    note_arr = model_output.get("note")

    all_pass_notes = []

    for pass_idx, (onset_t, frame_t, min_ms) in enumerate(pass_configs):
        print(f"[Stage 2B] Pass {pass_idx + 1}/{n_passes}:  "
              f"onset={onset_t}  frame={frame_t}  min_note={min_ms}ms")

        min_note_len = int(np.round(min_ms / 1000 * (AUDIO_SAMPLE_RATE / FFT_HOP)))
        midi_data, _ = bp_infer.model_output_to_notes(
            model_output,
            onset_thresh=onset_t,
            frame_thresh=frame_t,
            min_note_len=min_note_len,
            min_freq=BASS_HZ_MIN,
            max_freq=BASS_HZ_MAX,
            melodia_trick=False,
        )

        pass_notes = []
        for instrument in midi_data.instruments:
            for note in instrument.notes:
                pitch = int(note.pitch)
                if not (BASS_MIDI_MIN <= pitch <= BASS_MIDI_MAX):
                    continue
                confidence = _frame_confidence(note_arr, note.start, note.end, pitch)
                pass_notes.append({
                    "pitch":      pitch,
                    "start":      round(float(note.start), 4),
                    "duration":   round(float(note.end - note.start), 4),
                    "confidence": round(confidence, 4),
                })

        pass_notes.sort(key=lambda n: n["start"])
        pass_notes = _correct_octave_errors_bass(pass_notes)
        all_pass_notes.append(pass_notes)
        print(f"[Stage 2B] Pass {pass_idx + 1}: {len(pass_notes)} notes")

    # Merge all passes into a single deduplicated note list
    notes = _merge_passes_bass(all_pass_notes, note_arr)
    notes.sort(key=lambda n: n["start"])

    total_raw = sum(len(p) for p in all_pass_notes)
    print(f"[Stage 2B] Merged {total_raw} notes across {n_passes} pass(es) "
          f"-> {len(notes)} unique notes")

    # Stem energy gate: remove ghost notes from silent stem sections
    before = len(notes)
    notes  = gate_notes_by_stem_energy_for(notes, "bass")
    if before != len(notes):
        print(f"[Stage 2B] Stem energy gate: removed {before - len(notes)} ghost notes "
              f"({len(notes)} remaining)")

    # CREPE overlay — bass is monophonic, making it ideal for CREPE
    if BASS_CREPE_ENABLED:
        auto_repet_b = stem_conf < BASS_CREPE_REPET_AUTO_THRESHOLD
        use_repet_b  = BASS_CREPE_REPET_ENABLED or auto_repet_b
        if auto_repet_b and not BASS_CREPE_REPET_ENABLED:
            print(f"[Stage 2B] Auto-enabling REPET-SIM (stem_confidence={stem_conf:.2f} < {BASS_CREPE_REPET_AUTO_THRESHOLD})")
        try:
            crepe_notes = _extract_crepe_bass(audio_path, bass_style, use_repet=use_repet_b)
            notes = _merge_crepe_with_basic_pitch_bass(crepe_notes, notes)
        except ImportError:
            print("[Stage 2B] torchcrepe not installed — skipping CREPE overlay")
        except Exception as exc:
            print(f"[Stage 2B] CREPE failed ({exc}) — using basic-pitch only")

    _apply_sustained_boost_bass(notes, BASS_SUSTAINED_BOOST_MIN_S,
                                BASS_SUSTAINED_BOOST_AMOUNT, BASS_SUSTAINED_BOOST_CAP)

    if save:
        _save_bass(notes)
    return notes


def _resolve_thresholds_bass(bass_style: str, stem_conf: float) -> tuple[float, float, int]:
    row = BASS_PITCH_THRESHOLDS.get(bass_style, BASS_PITCH_THRESHOLDS["bass_fingered"])
    onset, frame, min_ms = row
    if onset is None:
        onset = round(BASS_PITCH_ONSET_BASE - stem_conf * BASS_PITCH_ONSET_SCALE, 2)
        frame = round(BASS_PITCH_FRAME_BASE - stem_conf * BASS_PITCH_FRAME_SCALE, 2)
    return onset, frame, min_ms


def _frame_confidence(note_arr, start_s: float, end_s: float, midi_pitch: int) -> float:
    if note_arr is None:
        return 0.5
    start_f   = int(start_s * BP_FRAMES_PER_SEC)
    end_f     = max(start_f + 1, int(end_s * BP_FRAMES_PER_SEC))
    pitch_idx = midi_pitch - BP_MIDI_OFFSET
    if not (0 <= pitch_idx < note_arr.shape[1]):
        return 0.5
    frames = note_arr[start_f:end_f, pitch_idx]
    return float(frames.mean()) if len(frames) > 0 else 0.5


def _correct_octave_errors_bass(notes: list[dict]) -> list[dict]:
    import bisect

    if len(notes) < 3:
        return notes

    corrected = list(notes)
    starts = [n["start"] for n in corrected]

    for i, note in enumerate(corrected):
        t = note["start"]
        lo = bisect.bisect_left(starts, t - 1.0)
        hi = bisect.bisect_right(starts, t + 1.0)
        neighbours = [corrected[j]["pitch"] for j in range(lo, hi) if j != i]
        if not neighbours:
            continue
        p = note["pitch"]
        min_dist = min(abs(p - nb) for nb in neighbours)
        if min_dist > 5:
            for shift in (12, -12):
                shifted = p + shift
                if not (BASS_MIDI_MIN <= shifted <= BASS_MIDI_MAX):
                    continue
                shifted_dist = min(abs(shifted - nb) for nb in neighbours)
                if shifted_dist < min_dist:
                    corrected[i] = dict(note, pitch=shifted)
                    break

    return corrected


# ── Multi-pass merge ──────────────────────────────────────────────────────────

def _merge_passes_bass(all_pass_notes: list[list[dict]], note_arr) -> list[dict]:
    import bisect

    if len(all_pass_notes) == 1:
        return _flat_merge_bass(all_pass_notes, note_arr)

    base_notes      = all_pass_notes[0]
    stricter_passes = all_pass_notes[1:]

    def _build_lookup(notes):
        return sorted((n["pitch"], n["start"]) for n in notes)

    stricter_lookups = [_build_lookup(p) for p in stricter_passes]

    def _is_confirmed(note, lookup):
        pitch = note["pitch"]
        t     = note["start"]
        lo = bisect.bisect_left( lookup, (pitch, t - BASS_PITCH_MERGE_PROXIMITY_S))
        hi = bisect.bisect_right(lookup, (pitch, t + BASS_PITCH_MERGE_PROXIMITY_S))
        return any(lookup[i][0] == pitch for i in range(lo, hi))

    confirmed_1 = confirmed_all = 0
    result = []

    for note in sorted(base_notes, key=lambda n: n["start"]):
        confirmations = sum(1 for lk in stricter_lookups if _is_confirmed(note, lk))
        if confirmations >= len(stricter_passes):
            weight = BASS_CONF_WEIGHT_STRONG
            confirmed_all += 1
        elif confirmations >= 1:
            weight = BASS_CONF_WEIGHT_CONFIRMED
            confirmed_1 += 1
        else:
            weight = BASS_CONF_WEIGHT_BASE

        ns = note["start"]
        ne = ns + note["duration"]
        frame_conf = _frame_confidence(note_arr, ns, ne, note["pitch"])
        result.append({
            "pitch":      note["pitch"],
            "start":      note["start"],
            "duration":   note["duration"],
            "confidence": round(frame_conf * weight, 4),
        })

    base_only = len(base_notes) - confirmed_1 - confirmed_all
    print(f"[Stage 2B] Confidence merge: {confirmed_all} strong "
          f"+ {confirmed_1} confirmed + {base_only} base-only "
          f"= {len(result)} notes")
    return result


def _flat_merge_bass(all_pass_notes: list[list[dict]], note_arr) -> list[dict]:
    pool = []
    for pass_notes in all_pass_notes:
        pool.extend(pass_notes)
    pool.sort(key=lambda n: (n["pitch"], n["start"]))

    if not pool:
        return []

    merged = [dict(pool[0])]
    for note in pool[1:]:
        prev     = merged[-1]
        prev_end = prev["start"] + prev["duration"]
        if note["pitch"] == prev["pitch"] and note["start"] <= prev_end + BASS_PITCH_MERGE_PROXIMITY_S:
            new_end = max(prev_end, note["start"] + note["duration"])
            merged[-1]["duration"] = round(new_end - prev["start"], 4)
        else:
            merged.append(dict(note))

    result = []
    for note in merged:
        ns = note["start"]
        ne = ns + note["duration"]
        frame_conf = _frame_confidence(note_arr, ns, ne, note["pitch"])
        note["confidence"] = round(frame_conf, 4)
        result.append(note)
    return result


# ── pyin fallback backend ─────────────────────────────────────────────────────

def _extract_pyin_bass(audio_path: str, save: bool = True) -> list[dict]:
    import librosa

    print(f"[Stage 2B] Loading audio: {audio_path}")
    y, sr = librosa.load(audio_path, sr=22050, mono=True)
    print(f"[Stage 2B] Audio loaded — {len(y)/sr:.1f}s @ {sr}Hz")
    print("[Stage 2B] Running pyin (monophonic fallback)...")

    f0, voiced_flag, voiced_prob = librosa.pyin(
        y, sr=sr, fmin=BASS_HZ_MIN, fmax=BASS_HZ_MAX,
        frame_length=2048, hop_length=512,
    )

    notes = _frames_to_notes_bass(f0, voiced_flag, voiced_prob, hop_s=512/sr)
    print(f"[Stage 2B] pyin extracted {len(notes)} notes in bass range")

    if save:
        _save_bass(notes)
    return notes


# ── CREPE monophonic overlay ──────────────────────────────────────────────────

def _extract_crepe_bass(audio_path: str, bass_style: str = "bass_fingered", use_repet: bool = False) -> list[dict]:
    """
    Run torchcrepe monophonic pitch tracking on the bass stem.
    Bass is monophonic, making it ideal for CREPE.
    BASS_CREPE_FMAX is set to 880 Hz — well within the model's valid range.
    """
    import torchcrepe
    import librosa
    import torch

    conf_thresh = BASS_CREPE_CONF_THRESHOLDS.get(bass_style, 0.70)

    y, sr = librosa.load(audio_path, sr=22050, mono=True)

    if use_repet:
        S_full   = np.abs(librosa.stft(y))
        S_filter = librosa.decompose.nn_filter(S_full, aggregate=np.median, metric="cosine")
        S_filter = np.minimum(S_full, S_filter)
        mask     = librosa.util.softmask(S_full - S_filter, 2.0 * S_filter, power=2)
        phase    = np.exp(1j * np.angle(librosa.stft(y)))
        y        = np.real(librosa.istft(mask * S_full * phase))
        print("[Stage 2B] CREPE: REPET-SIM foreground separation applied")

    hop_length_samples = 256
    hop_s = hop_length_samples / sr

    audio_tensor = torch.tensor(y[None], dtype=torch.float32)
    frequency, confidence = torchcrepe.predict(
        audio_tensor,
        sr,
        hop_length=hop_length_samples,
        fmin=BASS_HZ_MIN,
        fmax=BASS_CREPE_FMAX,
        model=BASS_CREPE_MODEL,
        return_periodicity=True,
        decoder=torchcrepe.decode.viterbi,
        device="cpu",
    )

    freq_arr = frequency.squeeze(0).numpy()
    conf_arr = confidence.squeeze(0).numpy()

    notes         = []
    current_pitch = None
    current_start = None
    current_probs = []
    last_voiced_t = None

    for i, (freq, conf) in enumerate(zip(freq_arr, conf_arr)):
        t = i * hop_s
        is_voiced = (
            conf >= conf_thresh
            and freq > 0
            and not np.isnan(freq)
            and BASS_HZ_MIN <= freq <= BASS_HZ_MAX
        )
        if is_voiced:
            midi = _hz_to_midi(freq)
            if not (BASS_MIDI_MIN <= midi <= BASS_MIDI_MAX):
                is_voiced = False

        if is_voiced:
            midi = _hz_to_midi(freq)
            gap  = (t - last_voiced_t) if last_voiced_t is not None else 0.0
            if midi == current_pitch and gap <= BASS_CREPE_MAX_GAP_S:
                current_probs.append(float(conf))
            else:
                if current_pitch is not None:
                    _finish_note(notes, current_pitch, current_start,
                                 last_voiced_t + hop_s, current_probs)
                current_pitch = midi
                current_start = t
                current_probs = [float(conf)]
            last_voiced_t = t
        else:
            if current_pitch is not None:
                gap = (t - last_voiced_t) if last_voiced_t is not None else 0.0
                if gap > BASS_CREPE_MAX_GAP_S:
                    _finish_note(notes, current_pitch, current_start,
                                 last_voiced_t + hop_s, current_probs)
                    current_pitch = None
                    current_start = None
                    current_probs = []
                    last_voiced_t = None

    if current_pitch is not None and current_start is not None:
        _finish_note(notes, current_pitch, current_start,
                     len(freq_arr) * hop_s, current_probs)

    notes = [n for n in notes if n["duration"] >= BASS_CREPE_MIN_NOTE_S]
    notes.sort(key=lambda n: n["start"])
    print(f"[Stage 2B] CREPE: {len(notes)} notes (conf>={conf_thresh}, style={bass_style})")
    return notes


def _merge_crepe_with_basic_pitch_bass(
    crepe_notes: list[dict], bp_notes: list[dict]
) -> list[dict]:
    """
    Merge CREPE notes with basic-pitch notes for bass.
    Additive-only: CREPE boosts confidence or adds missed pitches.
    """
    import bisect

    if not crepe_notes:
        return bp_notes

    bp_sorted = sorted(bp_notes, key=lambda n: n["start"])
    bp_starts  = [n["start"] for n in bp_sorted]

    boosted = added = 0
    bp_out  = [dict(n) for n in bp_sorted]

    for cn in crepe_notes:
        c_s    = cn["start"]
        c_e    = c_s + cn["duration"]
        c_p    = cn["pitch"]
        c_conf = cn["confidence"]

        lo = bisect.bisect_left(bp_starts,  c_s - 0.10)
        hi = bisect.bisect_right(bp_starts, c_e + 0.10)

        best_idx  = None
        best_conf = -1.0
        for idx in range(lo, hi):
            bp   = bp_out[idx]
            bp_s = bp["start"]
            bp_e = bp_s + bp["duration"]
            overlap = max(0.0, min(c_e, bp_e) - max(c_s, bp_s))
            if overlap <= 0:
                continue
            if abs(bp["pitch"] - c_p) <= BASS_CREPE_PITCH_TOLERANCE and bp["confidence"] >= 0.10:
                if bp["confidence"] > best_conf:
                    best_conf = bp["confidence"]
                    best_idx  = idx

        if best_idx is not None:
            if c_conf > bp_out[best_idx]["confidence"]:
                bp_out[best_idx] = dict(bp_out[best_idx], confidence=round(c_conf, 4))
                boosted += 1
        else:
            bp_out.append(dict(cn))
            bp_starts.append(c_s)
            added += 1

    bp_out.sort(key=lambda n: n["start"])
    print(f"[Stage 2B] CREPE merge: boosted {boosted} bp notes, added {added} new "
          f"= {len(bp_out)} total")
    return bp_out


# ── pyin frame segmentation ───────────────────────────────────────────────────

def _hz_to_midi(freq_hz: float) -> int:
    return int(round(69 + 12 * np.log2(freq_hz / 440.0)))


def _frames_to_notes_bass(f0, voiced_flag, voiced_prob, hop_s) -> list[dict]:
    notes = []
    current_pitch = None
    current_start = None
    current_probs = []

    for i, (freq, is_voiced, prob) in enumerate(zip(f0, voiced_flag, voiced_prob)):
        t = i * hop_s
        if is_voiced and freq is not None and not np.isnan(freq):
            midi = _hz_to_midi(freq)
            if not (BASS_MIDI_MIN <= midi <= BASS_MIDI_MAX):
                is_voiced = False

        if is_voiced and freq is not None and not np.isnan(freq):
            midi = _hz_to_midi(freq)
            if midi == current_pitch:
                current_probs.append(float(prob))
            else:
                if current_pitch is not None:
                    _finish_note(notes, current_pitch, current_start, t, current_probs)
                current_pitch = midi
                current_start = t
                current_probs = [float(prob)]
        else:
            if current_pitch is not None:
                _finish_note(notes, current_pitch, current_start, t, current_probs)
                current_pitch = None
                current_start = None
                current_probs = []

    if current_pitch is not None and current_start is not None:
        _finish_note(notes, current_pitch, current_start, len(f0) * hop_s, current_probs)

    notes = [n for n in notes if n["duration"] >= BASS_PYIN_MIN_NOTE_DURATION_S]
    notes.sort(key=lambda n: n["start"])
    return notes


def _finish_note(notes, pitch, start, end, probs):
    notes.append({
        "pitch":      pitch,
        "start":      round(start, 4),
        "duration":   round(end - start, 4),
        "confidence": round(float(np.mean(probs)) if probs else 0.0, 4),
    })


def _apply_sustained_boost_bass(
    notes: list[dict],
    min_s: float,
    amount: float,
    cap: float,
) -> None:
    boosted = 0
    for note in notes:
        if note["duration"] >= min_s:
            note["confidence"] = min(cap, note["confidence"] + amount)
            boosted += 1
    if boosted:
        print(f"[Stage 2B] Sustained boost applied to {boosted} notes")


# ── I/O helpers ───────────────────────────────────────────────────────────────

def _save_bass(notes: list[dict]) -> None:
    out_path = os.path.join(get_instrument_dir("bass"), "02_raw_notes.json")
    with open(out_path, "w") as f:
        json.dump(notes, f, indent=2)
    print(f"[Stage 2B] Saved -> {out_path}")


def load_raw_notes_bass() -> list[dict]:
    path = os.path.join(get_instrument_dir("bass"), "02_raw_notes.json")
    with open(path) as f:
        return json.load(f)
