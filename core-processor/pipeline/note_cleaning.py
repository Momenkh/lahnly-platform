...
def clean_notes(
    ...
    if bpm:
        merge_gap = min(CLEANING_MAX_MERGE_GAP_S, round(60.0 / (bpm * 8), 4))
    else:
    ...
    # 2. Adaptive confidence filter (pitch-aware: high notes get a lower gate)
    high_thresh = max(0.0, conf_thresh - CLEANING_HIGH_PITCH_CONF_REDUCTION)
    before = len(notes)
    notes  = [
        n for n in notes
        if n["confidence"] >= (high_thresh if n["pitch"] >= CLEANING_HIGH_PITCH_FLOOR else conf_thresh)
    ]
    print(f"[Stage 3] After confidence filter  : {len(notes):4d}  (removed {before - len(notes)})"
          f"  [gate: {conf_thresh} / high-pitch gate: {high_thresh}]")
    ...
    # 6. Polyphony limit (confidence + fret-aware)
    before = len(notes)
    ...
    if save:
        ...
        out_path = os.path.join(get_outputs_dir(), "03_cleaned_notes.json")
...
def load_clean_meta() -> dict:
    path = os.path.join(get_outputs_dir(), "03_clean_meta.json")
    try:
        with open(path) as f:
            return json.load(f)
...
def _load_bpm() -> float | None:
    ...
    path = os.path.join(get_outputs_dir(), "04_tempo.json")
    try:
        with open(path) as f:
...
def load_cleaned_notes() -> list[dict]:
    path = os.path.join(get_outputs_dir(), "03_cleaned_notes.json")
    with open(path) as f:
        return json.load(f)
